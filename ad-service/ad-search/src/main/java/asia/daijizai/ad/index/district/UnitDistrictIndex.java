...
public class UnitDistrictIndex implements IndexAware<String, Set<Long>> {
    ...
    public void add(String key, Set<Long> value) {
        ...
        Set<Long> unitIds = CommonUtil.getOrCreate(key, districtUnitMap, ConcurrentSkipListSet::new);
        unitIds.addAll(value);

        for (Long unitId : value) {

            Set<String> districts = CommonUtil.getOrCreate(unitId, unitDistrictMap, ConcurrentSkipListSet::new);
            districts.add(key);
        }
    ...
    public boolean match(Long adUnitId, List<DistrictFeature.ProvinceAndCity> districts) {
        if (unitDistrictMap.containsKey(adUnitId)
            ...
            List<String> targetDistricts = districts.stream()
                    .map(d -> CommonUtil.stringConcat(d.getProvince(), d.getCity()))
